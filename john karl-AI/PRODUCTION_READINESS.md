...
| Question | Decision |
| --- | --- |
| Generation timing | Generated **once, at save time**, and cached. Not regenerated on every detail-screen view. **Superseded post-launch** (see "Backend Integration Status" below): the backend now also regenerates live on every detail-screen view — save-time generation still happens as a pre-warm/fallback, but is no longer the only trigger. |
...
3. ~~When rendering the memory detail screen, call `GET {AI_SERVICE_URL}/api/v1/ai/memory-quote/{memory_id}`.~~ **Changed post-launch**: `GET /api/v1/memory-vault/{memoryId}/quote` now calls the AI service's `POST /api/v1/ai/memory-quote` (live regenerate) on every view instead of the cache-only `GET`, per the user's explicit decision to trade cache-freshness for a new quote on every detail-screen open. The AI service's cache-only `GET` endpoint itself is unchanged and still exists — the backend just no longer calls it from this code path. See "Backend Integration Status" below for the as-built detail and the cost/latency trade-off this implies.
...
- `src/utils/ai-service.client.ts` — HTTP client: `requestMemoryChat`, `requestMemoryQuoteGeneration` (calls the AI service's live-regenerate `POST`), `triggerMemoryQuoteGeneration` (fire-and-forget wrapper around the same POST, logs on failure, never throws). The earlier cache-only `fetchCachedMemoryQuote` helper was removed after the read-path change below made it dead code.
...
- `memory-vault.service.ts` — `createMemory` and `updateMemory` still call `triggerMemoryQuoteGeneration` after the audit log write (save-time pre-warm, async/best-effort, unchanged from the original Phase 2 data contract). **`getMemoryQuote(user, params)` was changed post-launch**: it now calls `requestMemoryQuoteGeneration` (the live-regenerate `POST`) directly and awaits the result, instead of reading the cache-only `GET`. This was an explicit, deliberate decision the user made after seeing the feature live — every open of a memory's detail screen now gets a freshly-generated quote rather than the one generated at save time. Trade-offs accepted knowingly: each view is now a real Claude API call (cost per view, not just per save/edit), and latency changed from a ~5ms cache read to a live model round-trip. A failed AI-service call now surfaces as a real error (502/503) on this endpoint rather than a silent `null`.
- `GET /api/v1/memory-vault/{memoryId}/quote` — route/controller unchanged in shape (same auth/permission model as the other memory-vault routes, same `{ pullQuote, commentary }` response), but its underlying behavior changed as described above — both fields are populated on success now rather than possibly `null`.
...
- Tests: `tests/memory-vault.service.test.ts` (location-required rejection, successful photo creation triggering quote generation, live quote regeneration on view, AI-service error propagation from the quote endpoint) and `tests/memory-vault.routes.test.ts` (auth-required + happy-path for the quote route) extended; new `tests/memory-chat.service.test.ts` (retrieval scoping to own vs. accepted family member's memories, 403 rejection for non-family members, conversation-id shape, citation mapping) and `tests/memory-chat.routes.test.ts` (auth, validation, happy path) added.
